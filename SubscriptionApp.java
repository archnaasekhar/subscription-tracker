...
public class SubscriptionApp extends JFrame {
    ...
    private static HashMap<String, ArrayList<Subscription>> userSubscriptions = new HashMap<>();
    ...
    private void showLoginPage() {
        ...
        JPanel panel = new JPanel(new BorderLayout()) {
            ...
            protected void paintComponent(Graphics g) {
                ...
                setBackground(new Color(224, 240, 255)); // Light blue
            }
        };
        ...
        title.setForeground(new Color(0, 51, 102)); // Dark navy
        ...
        JButton loginBtn = createStyledButton("Login", new Color(0, 102, 204)); // Blue
        JButton createBtn = createStyledButton("Create Account", new Color(51, 153, 255)); // Lighter Blue
    ...
    private void showCreateAccountPage() {
        ...
        JTextField userField = new JTextField();
        JPasswordField passField = new JPasswordField();
        JButton createBtn = createStyledButton("Create", new Color(0, 102, 204)); // Blue
        ...
        createBtn.addActionListener(e -> {
            ...
            if (users.containsKey(user)) {
                ...
                users.put(user, pass);
                userSubscriptions.put(user, new ArrayList<>());
                saveData();
    ...
    private void openDashboard() {
        ...
        content.setBackground(new Color(224, 240, 255)); // Light blue
    ...
    private void showAddForm(JPanel content) {
        ...
        addBtn.addActionListener(e -> {
            try {
                ...
                userSubscriptions.get(currentUser).add(sub);
                saveData();
    ...
    private void showList(JPanel content) {
        ...
        ArrayList<Subscription> subs = userSubscriptions.get(currentUser);
        if (subs.isEmpty()) {
    ...
    private void showDelete(JPanel content) {
        ...
        delBtn.addActionListener(e -> {
            String name = nameField.getText().trim();
            ArrayList<Subscription> subs = userSubscriptions.get(currentUser);
            boolean removed = subs.removeIf(s -> s.getName().equalsIgnoreCase(name));
            if (removed) {
                saveData();
    ...
    private void showTotalCost(JPanel content) {
        content.removeAll();
        double total = 0;
        for (Subscription s : userSubscriptions.get(currentUser)) total += s.getCost();
    ...
    private void saveData() {
        ...
        try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(SUBS_FILE))) {
            out.writeObject(userSubscriptions);
        } catch (Exception ignored) {}
    }
    ...
    private void loadData() {
        ...
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(SUBS_FILE))) {
            userSubscriptions = (HashMap<String, ArrayList<Subscription>>) in.readObject();
        } catch (Exception ignored) {}
    }
}
...
class Subscription implements Serializable {
    private String name, category, renewalDate, frequency;
    private double cost;

    public Subscription(String name, String category, double cost, String renewalDate, String frequency) {
        this.name = name;
